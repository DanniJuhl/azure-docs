...
As Always On Availability Groups is supported for SAP on-premises (see SAP Note [1772688]), it's supported in combination with SAP in Azure. Special considerations may apply regarding the Availability Group Listener which requires an Azure Load Balancer if a DNN is not utilized. 
Therefore, some different installation steps are necessary to overcome the specific behavior of Azure.
